  ...
  reducers: {
    ...
    signIn: (state, action) => {
      ...
      if (!checkData(state, userData.Email, userData.Password)) {
        ...
        if (user) {
          if (user.Password === userData.Password) state.currentUser = user;
          else {
            state.errors.passwordError.hasError = true;
            state.errors.passwordError.message =
              "Password does not match! If you forgot your password, please reset it.";
          }
        } else {
    ...
    signUp: (state, action) => {
      ...
      if (!checkData(state, newUser.Email, newUser.Password)) {
        if (
          !state.users.find((user) => user.Email === newUser.Email) &&
